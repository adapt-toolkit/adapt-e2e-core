...
fn two_sessions_have_distinct_ids_and_do_not_cross_decrypt() {
    ...
    assert_ne!(id_ab, id_ac, "distinct conversations have distinct session ids");
...
fn skipped_key_store_is_session_local() {
    ...
    assert_eq!(p2, b"two", "Bob's session-local skip store decrypts m2 out of order");
}
